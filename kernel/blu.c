...
{
    void* file_buf = malloc(262144);
    if (!file_buf) return -1;

    int fd = vfs_open(path, 0);
    if (fd < 0) { free(file_buf); return -1; }

    int file_size = vfs_read(fd, file_buf, 262144);
    vfs_close(fd);
    if (file_size < (int)sizeof(blu_header_t)) { free(file_buf); return -1; }

    blu_header_t* hdr = (blu_header_t*)file_buf;
    ...
    if (!image) { free(file_buf); return -1; }
    memset(image, 0, image_size);

    if (code_in_file > 0)
        memcpy(image, (uint8_t*)file_buf + sizeof(blu_header_t), code_in_file);
